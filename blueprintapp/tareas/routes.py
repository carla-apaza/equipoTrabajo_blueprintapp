...
def edit(id):
    # Busca la tarea o lanza un error 404 si no existe
    tarea = Tarea.query.get_or_404(id)

    if request.method == "POST":
        tarea.descripcion = request.form.get("descripcion")
        tarea.completado = request.form.get("completado") is not None

        db.session.commit()  # Guardamos los cambios en el objeto ya existente
        return redirect(url_for("bp_tarea.index"))
...
# === NUEVA RUTA: ELIMINAR ===
@bp_tarea.route("/delete/<int:id>", methods=["POST"])
def delete(id):
    tarea = Tarea.query.get_or_404(id)
