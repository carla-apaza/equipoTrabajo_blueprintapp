...
def edit(id):
    # Buscamos al miembro por su ID principal o devolvemos error 404 si no existe
    ...
    if request.method == "GET":
        ...
        db.session.commit()  # Guardamos los cambios efectuados sobre el objeto
        return redirect(url_for("bp_miembro.index"))


# --- OPERACIÓN ELIMINAR ---
@bp_miembro.route("/delete/<int:id>", methods=["POST"])
def delete(id):
    miembro = Miembro.query.get_or_404(id)
    db.session.delete(miembro)  # Eliminamos el registro
    db.session.commit()
    return redirect(url_for("bp_miembro.index"))
